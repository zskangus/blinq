...
- (void)initialization {
    ...
    [_pointImage imageWithColor:[UIColor blueColor]];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGPoint center = CGPointMake(375/2.0, 375/2.0);
    ...
    CGFloat start = _startAngle + (CircleDegreeToRadian(360));//起点
    ...
    //画小圆点
    if (_showPoint) {
       // CGContextDrawImage(ctx, CGRectMake(CircleSelfWidth/2 + ((275-_strokeWidth)/2.f-1)*cosf(valueEndA)-_strokeWidth/2.0, CircleSelfWidth/2 + ((275-_strokeWidth)/2.f-1)*sinf(valueEndA)-_strokeWidth/2.0, 30, 30), [_pointImage imageWithColor:_pointColor].CGImage);
        
        CGContextDrawImage(ctx, CGRectMake(CircleSelfWidth/2 + ((292.5-25)/2.f-1)*cosf(valueEndA)-25/2.0, CircleSelfWidth/2 + ((292.5-25)/2.f-1)*sinf(valueEndA)-25/2.0, 25, 25), [_pointImage imageWithColor:_pointColor].CGImage);

        CGContextDrawImage(ctx, CGRectMake(CircleSelfWidth/2 + ((292.5-25)/2.f-1)*cosf(start)-25/2.0, CircleSelfWidth/2 + ((292.5-25)/2.f-1)*sinf(start)-25/2.0, 25, 25), [_pointImage imageWithColor:_pointColor].CGImage);

        
        //CGContextDrawImage(ctx, CGRectMake(CircleSelfWidth/2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-1)*cosf(start)-_strokeWidth/2.0, CircleSelfWidth/2 + ((CGRectGetWidth(self.bounds)-_strokeWidth)/2.f-1)*sinf(start)-_strokeWidth/2.0, _strokeWidth, _strokeWidth), [_pointImage imageWithColor:_pointColor].CGImage);
    }
...
- (void)setProgressWithValue:(CGFloat)progress{
